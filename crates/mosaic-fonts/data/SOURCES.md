...
on 2026-05-11 from the upstream `notofonts/notofonts.github.io`
...
(2024-11-21). The license text under `LICENSE-Noto-Sans` was
downloaded from `notofonts/latin-greek-cyrillic` at commit
...
The URLs below are pinned to those upstream commit SHAs so re-vendoring
fetches the exact same bytes regardless of how the upstream `main`
branch drifts. The per-file `SHA-256` column is the load-bearing
integrity check; the URLs are documentary.
