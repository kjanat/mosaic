...
- Safe fix-its for unsafe portable paths (https://github.com/kjanat/mosaic/issues/128): `MOS0049`
  ...
  as a quick fix. The new `mos_core::portable_path_fix` only offers the `\`→`/` swap when the result
  is a relative path the resolver accepts; rooted, drive-prefixed (`C:\...`), and UNC forms keep the
  error without a guess. Manifest `[project].entry` / `[output].pdf` paths still report `MOS0049`
  without a fix because the CLI has no source span for them.
